...
def import_the_snake():
    ...
    import src.game.main  # noqa: F401
...
def _the_snake(snake_import_test):
    """Загружает классы из game_objects."""
    module = {}
    try:
        module["GameObject"] = __import__(
            "src.game_objects.game_object", fromlist=["GameObject"]
        ).GameObject
        module["Snake"] = __import__(
            "src.game_objects.snake", fromlist=["Snake"]
        ).Snake
        module["Apple"] = __import__(
            "src.game_objects.apple", fromlist=["Apple"]
        ).Apple
